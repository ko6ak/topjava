...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        List<UserMealWithExcess> recursive = recursive(meals, LocalTime.of(7, 0), LocalTime.of(12, 0), 2000);
    ...
    static Map<LocalDate, Integer> caloriesPerDate = new HashMap<>();
    static int count = 0;

    public static List<UserMealWithExcess> recursive(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        if (Objects.isNull(meals) || Objects.isNull(startTime) || Objects.isNull(endTime) || caloriesPerDay < 0) return new ArrayList<>();
        UserMeal userMeal = meals.get(count++);
        caloriesPerDate.merge(userMeal.getDateTime().toLocalDate(), userMeal.getCalories(), Integer::sum);
        List<UserMealWithExcess> result;
        if (count != meals.size()) result = recursive(meals, startTime, endTime, caloriesPerDay);
